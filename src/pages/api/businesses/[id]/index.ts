...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case 'PUT':
      return updateBusinessById();
    case 'DELETE':
      return deleteBusinessById();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getBusinessById() {
    const data = await prisma.business.findFirst(convertQueryToPrismaUtil(req.query, 'business'));
  ...
  async function deleteBusinessById() {
    const data = await prisma.business.delete({
      where: { id: req.query.id as string },
    });
    return res.status(200).json(data);
  }
